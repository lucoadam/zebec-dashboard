...
import NftTransactionsTableRow from "./NftTransactionsTableRow"
...
export const NftTransactionsTable: FC<TransactionTableProps> = ({
  transactions,
  fetchTransactions
}) => {
          ...
          {transactions.results.length === 0 && !loading ? (
            <tr>
              <td colSpan={headers.length}>
                <EmptyDataState
                  message={t(
                    "transactions:table.treasury-nfts-transactions-empty"
                  )}
                />
              </td>
            </tr>
